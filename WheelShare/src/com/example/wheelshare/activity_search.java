...
public class activity_search extends activity_base {
	...
	private boolean fieldErrorCheck() {
		...
		if(sourceStr.equals(destinationStr)){
			DestinationCheck=false;SourceCheck=false;}
		else
		{
			DestinationCheck=true;SourceCheck=true;
		}
		...
		if (dayStr == null || monthStr == null || yearStr == null ) {
			...
			int month1= Integer.parseInt(monthStr);
			int year1= Integer.parseInt(yearStr);
			
			Calendar c = Calendar.getInstance();
			int checkdate = c.get(Calendar.YEAR) * 100 + c.get(Calendar.MONTH);
			if((day1< c.get(Calendar.DAY_OF_MONTH)) && (month1 < c.get(Calendar.MONTH)) && (year1 < c.get(Calendar.YEAR)))
			{
				DateCheck = false;
			}
			if (date - checkdate > 6) {
