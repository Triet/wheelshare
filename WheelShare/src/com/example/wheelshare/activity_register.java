...
public class activity_register extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(MainActivity.hindi)
			setContentView(R.layout.activity_register_h);	
			else
...
private boolean fieldErrorCheck() { 
        ...
        if (userNameCheck = Pattern.matches("^[a-z0-9_-]{3,15}$", userName)) {userNameRegText.setTextColor(Color.BLACK); userNameCheck = true;}
        else { userNameRegText.setTextColor(getResources().getColor(R.color.textErrorColor));   
		userNameCheck = false;
        }
  
        if (passWordCheck = Pattern.matches("((?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%]).{6,20})", passWord)) { passWordRegText.setTextColor(Color.BLACK);  passWordCheck = true; }
        else {passWordRegText.setTextColor(getResources().getColor(R.color.textErrorColor)); 
		passWordCheck = false;}
          
        if (cpassWordCheck =  (cpassWord.equals(passWord))){cpassWordRegText.setTextColor(Color.BLACK); 	cpassWordCheck = true;}
        else{ cpassWordRegText.setTextColor(getResources().getColor(R.color.textErrorColor)); cpassWordCheck = false;}
  
        if (firstNameCheck = Pattern.matches("^[a-z0-9_-]{3,15}$", firstName)) {firstNameRegText.setTextColor(Color.BLACK);firstNameCheck = true;
		 }
        else {firstNameRegText.setTextColor(getResources().getColor(R.color.textErrorColor)); 		firstNameCheck = false;}
        
        if (lastNameCheck = Pattern.matches("^[a-z0-9_-]{3,15}$", lastName)){ lastNameRegText.setTextColor(Color.BLACK);	lastNameCheck = true; }
        else{ lastNameRegText.setTextColor(getResources().getColor(R.color.textErrorColor));	lastNameCheck = false; }
       
        if (genderCheck = gender != ""){genderRegText.setTextColor(Color.BLACK); 	genderCheck = true;}
        else { genderRegText.setTextColor(getResources().getColor(R.color.textErrorColor)); 	genderCheck = false;} 

        // all checks pass, return true 
            return (userNameCheck && firstNameCheck && lastNameCheck && passWordCheck && cpassWordCheck && genderCheck); 
...
/*
	...
	private boolean fieldErrorCheck() {
		boolean userNameCheck, passWordCheck, cpassWordCheck, firstNameCheck, lastNameCheck, creditCardCheck, genderCheck;
		...
		if (creditCardNum.equals("")) {
			creditCardRegText.setTextColor(getResources().getColor(
					R.color.textErrorColor));
			creditCardCheck = false;
		} else {
			creditCardRegText.setTextColor(Color.BLACK);
		...
		if (userNameCheck && firstNameCheck && lastNameCheck && passWordCheck
				&& cpassWordCheck && creditCardCheck && genderCheck)
			return true;
		else
			return false;
